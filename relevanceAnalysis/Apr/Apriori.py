...
class Apriori:
    ...
    def genAssociations(self):
        candidate = {}  # dictionary type
        count = {}

        self.F[1] = self.firstPass(self.freqList, 1)  # maybe it means the time the words appeared is one.
        k = 2
        while len(self.F[k - 1]) != 0:  # candidate's list
        ...
        for k, itemset in F.iteritems:
            if k >= 2:
                for item in itemset:
                    ...
                    for subset in subsets:
                        if len(subset) == 1:
                            subCount = self.freqList[subset[0]]  # support count
                        else:
...
def main():
    ...
    for k, item in frequentItemsets.iteritems():
        for i in item:
            if k >= 2:
                count += 1
                print(count, ": ", readable(i, goods), "\tsupport=", a.freqList[i])

    print("Skyline Itemsets: ", count)
    if not noRules:
        rules = a.genRules(frequentItemsets)
        for i, rule in enumerate(rules):
            print("Rule", i + 1, ":\t", readable(rule[0], goods), "\t-->", readable(rule[1], goods), "\t [sup=",
                  rule[2], " conf=", rule[3], "]")
