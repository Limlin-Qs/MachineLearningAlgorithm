...
class ImprovedMinMaxKMeans:
    def __init__(self, n_clusters=3, p_max=0.5, p_step=0.01, beta=0.1, variance_threshold=10 ** -6, max_iter=500,
                 verbose=0):
    ...
    def update_weights(self, cluster_assignments, X):
        ...
        for k in range(self.n_clusters):
            ...
            self.weights_[k] = self.beta * self.weights_[k] + (1 - self.beta) * np.power(variance, 1 / (
                        1 - self.p_)) / total_variance
    ...
    def compute_minimization_step(self, instance):
        distances = []
        for i in range(self.n_clusters):
            distance = np.power(self.weights_[i], self.p_) * np.power(
                np.linalg.norm(instance - self.cluster_centers_[i]), 2)
